...
namespace HT {
    ...
    BOOL Insert(const HTHANDLE* hthandle, const Element* element)
    {
        ...
        if (strlen((char*)element->key) > element->keylength)
        {
            strcpy_s(ht->LastErrorMessage, "Key longer then max size");
            return FALSE;
        }
        ...
        for (int i = 0; i < ht->Capacity; i++) {
            ...
            {
                strcpy_s(ht->LastErrorMessage, "This key exists");
                return FALSE;
            }
        ...
        int newpayloadlength)
    {
        ...
        if (strlen((char*)newpayload) > newpayloadlength) {
            strcpy_s(ht->LastErrorMessage, "New payload too long");
            return FALSE;
        }
